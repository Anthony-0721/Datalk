...
class SchemaIndex:
    ...
    def _build_raw_documents(self) -> list[dict[str, Any]]:
        ...
        database_names = sorted({table.get("database", "datalk_mock") for table in SCHEMA})
        for database in database_names:
            # 每个数据库复用一个 DuckDB 连接构建字段文档。
            with self.data_engine.connect(database) as connection:
                for table in (item for item in SCHEMA if item.get("database", "datalk_mock") == database):
    ...
    def _append_table_documents(
        ...
        for field in table["fields"]:
            ...
            keyword_text = " ".join([
                table.get("database", "datalk_mock"), table.get("domain", ""),
            ...
            output.append({
                ...
                "database_id": table.get("database", "datalk_mock"),
                "table_id": table["id"],
