...
<template>
  <div v-if="!authReady" class="auth-loading"><span></span><p>正在连接 Datalk…</p></div>

  <main v-else-if="!authUser" class="login-page">
    <section class="login-intro">
      <div class="login-brand"><span>D</span><strong>Datalk</strong></div>
      <div>
    ...
    <section class="login-side">
      <form class="login-card" @submit.prevent="loginUser">
        <header><span class="login-mark">D</span><div><h2>欢迎回来</h2><p>登录 Datalk</p></div></header>
        <label>
          <span>账号</span>
  ...
  <div v-else class="app-layout">
    <aside class="history-sidebar" :class="{ open: leftOpen }">
      <div class="brand-row">
        ...
        <div><strong>Datalk</strong><small>AI 数据分析</small></div>
      </div>
            ...
            <div class="assistant-message">
              ...
              <div class="message-body assistant-body">
                <div class="answer-heading" :class="{ 'qa-heading': turn.result.route !== 'database_query' }">
                  <div><small>Datalk</small><strong v-if="turn.result.route === 'database_query'">{{ turn.result.status === "failed" ? "处理失败" : turn.result.status === "waiting_clarification" ? "需要补充信息" : "查询完成" }}</strong></div>
